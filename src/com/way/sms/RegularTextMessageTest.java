...
public class RegularTextMessageTest {
	...
	public void identifies_a_way_request() throws Exception {
		...
		assertThat(new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), EasyMock.createMock(TextMessage.class)).isWayRequest(), is(true));
		verify(smsMessage);
	}
	...
	public void identifies_a_where_are_you_request() throws Exception {
		...
		final SmsMessage smsMessage = mockMessage("where are you");
		assertTrue(new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), EasyMock.createMock(TextMessage.class)).isWayRequest());
		verify(smsMessage);
	}
	...
	public void responds_to_way() throws Exception {
		...
		assertThat("Should have responded to 'way'", new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), EasyMock.createMock(TextMessage.class)).isWayRequest(),
				is(true));
		verify(smsMessage);
	...
	public void does_not_respond_to_w() throws Exception {
		...
		assertThat("Should not have replied to just W", new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), null).isWayRequest(), is(false));
		verify(smsMessage);

	}

	@Test
	public void delegates_to_next_handler_if_not_its_own_request() throws Exception {
		final SmsMessage smsMessage = mockMessage("W");

		final TextMessage textMessageHandler = EasyMock.createMock(TextMessage.class);
		expect(textMessageHandler.isWayRequest()).andReturn(true);
		replay(textMessageHandler);

		assertThat("The delgatee should have replied in the affirmative", new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), textMessageHandler).isWayRequest(),
				is(true));
		verify(smsMessage);
	...
	public void identifies_a_non_way_request() throws Exception {
		...
		assertFalse(new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), EasyMock.createMock(TextMessage.class)).isWayRequest());
		verify(smsMessage);
	...
	public void returns_the_senders_address() throws Exception {
		...
		final RegularTextMessage waySms = new RegularTextMessage(smsMessage, EasyMock.createMock(Locator.class), new WayRequest(), EasyMock.createMock(TextMessage.class));
	...
	public void generates_a_reply_for_the_way_request() throws Exception {
		...
		final RegularTextMessage waySms = new RegularTextMessage(smsMessage, locator, new WayRequest(), EasyMock.createMock(TextMessage.class));
	...
	public void delegates_generate_reply_to_next_handler_if_not_its_own_request() throws Exception {
		...
		final RegularTextMessage waySms = new RegularTextMessage(smsMessage, locator, new WayRequest(), textMessageHandler);
